...
title:    " 《强化学习》读书笔记"
subtitle:   "简要总结，不是教程"
date:     2019-1２-２８ 15:32:24.000000000 +09:00
author:     "Jason"
...
收敛条件$\sum_{n=1}^{\infin}\alpha_n(a)=\infin,\sum_{n=1}^{\infin}\alpha_n(a)^2<\infin$
...
$G_t= R_{t+1} + \gamma R_{t+2} + \gamma^2 R_{t+3} + ··· =\sum_{k=0}^{\infin }\gamma^k R_{t+k+1},\gamma\in [0,1]$

$\gamma$ 决定agent是myopic还是farsighted

$G_t = R_{t+1}+\gamma G_{t+1}$

$G_t$是收敛的，$\sum_{k=0}^{\infin}\gamma^k=\frac{1}{1-\gamma}$
...
$v_\pi$是不动点，根据压缩映射原理，保证收敛$v_{k}=v_\pi,k\to \infin$
...
不用每个state 都更新infinite次，关注和策略相关的一个state子集
...
$r(\pi)=\underset{h\to \infin}{lim}\frac{1}{h}\sum_{t=1}^{h}E[R_t|S_0,A_{0:t-1} \sim \pi]$

$=\underset{t\to \infin}{lim}E[R_t|S_0,A_{0:t-1} \sim \pi]$

$=\sum_s \mu_\pi(s)\sum_a \pi{(a|s)} \sum_{s',r}p(s',r|s,a)r$

$\mu_\pi(s)=lim_{t\to\infin}Pr(S_t=s|A_{0:t-1} \sim \pi)$
...
$G_t^\lambda = (1-\lambda)\sum_{n=1}^{\infin}\lambda^{n-1}G_{t:t+n}$
